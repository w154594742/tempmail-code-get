...
- **标签化显示**: 图标+中文标签清晰展示信息（📧 邮箱、🕒 时间、🔢 验证码等）
...
### v1.5.0 (当前版本)
- ✅ **邮件内容历史功能**: 新增完整的邮件内容历史记录管理
  - 保存获取验证码时的完整邮件内容（主题、正文、HTML格式）
  - 支持邮件详情查看，包含发送邮箱、时间戳、验证码等信息
  - 智能HTML邮件渲染，支持渲染视图和源代码视图切换
  - 使用iframe安全渲染HTML内容，防止恶意脚本执行
